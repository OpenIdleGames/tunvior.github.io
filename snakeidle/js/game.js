...
var Game = function () {
  function Game() {
    ...
    this.upgrades = [];
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Fruit value", "Double the value of fruits", "this.game.fruitValue = this.game.fruitValue * 2", 1000, 8, 100));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Snake length bonus", "Increase the bonus from snake length by 1", "this.game.snakeLengthBonus = this.game.snakeLengthBonus + 1", 10, 90, 100));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Golden fruit chance", "Increase the chance for golden fruit spawn by 10%", "this.game.goldenFruitChance = this.game.goldenFruitChance + 10", 1000, 1000, 10));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Golden fruit value", "Increase the value for golden fruits by 1", "this.game.goldenFruitValue = this.game.goldenFruitValue + 1", 100000, 75, 100));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Purple fruit chance", "Increase the chance for purple fruit spawn by 0.5%", "this.game.purpleFruitChance = this.game.purpleFruitChance + 0.5", 1000000, 1000, 10));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Purple fruit duration", "Increase duration of purple fruit effect by 1 tick", "this.game.purpleFruitDuration = this.game.purpleFruitDuration + 1", 1500000, 6, 100));
    this.upgrades.push(new Upgrade(this, this.upgrades.length, "Purple fruit multiplier", "Increase multiplier of purple fruit effect by 5", "this.game.purpleFruitEffect = this.game.purpleFruitEffect + 5", 3000000, 8, 100));
  ...
  _createClass(Game, [{
    key: "updateFieldButton",
    value: function updateFieldButton() {
    ...
    key: "drawUpgrade",
    value: function drawUpgrade(upgrade, prog) {
    ...
    key: "eraseField",
    value: function eraseField(index) {
    ...
    key: "eraseUpgrade",
    value: function eraseUpgrade(index) {
      this.buttonsHandler.deleteButton(this.upgrades[index].buyButton);
      $("#upgrade" + index).remove();
    }
  }, {
    key: "calculateNextFieldCost",
    value: function calculateNextFieldCost() {
    ...
    key: "addFruits",
    value: function addFruits(snakeLength, goldenFruit, purpleFruit) {
    ...
    key: "removeFruits",
    value: function removeFruits(number) {
    ...
    key: "prestigeField",
    value: function prestigeField(prog) {
      var index = 0;
    ...
    key: "heuristic",
    value: function heuristic(ticks) {
      var fruitsEarned = 0;
      for (var i = 0; i < this.fields.length; i++) {
        ...
        var gain = (((average - 1) * this.snakeLengthBonus + 1) * this.fruitValue * this.multiplier * (1 - this.goldenFruitChance * 1.0 / 100) + ((average - 1) * this.snakeLengthBonus + 1) * this.fruitValue * this.multiplier * this.goldenFruitValue * this.goldenFruitChance * 1.0 / 100) * chance * ticks;
        gain = Math.ceil(gain);
        fruitsEarned += gain;
      }
    ...
    key: "offlineProgress",
    value: function offlineProgress(lastTimestamp) {
      var now = Date.now();
